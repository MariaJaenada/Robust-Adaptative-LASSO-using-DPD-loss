...
HGIC.dpdLASSO_LR <- function(X,Y, beta.init,
  ...
  if(lmode=="lambda0"){
    ...
    lambdamax0<-LS_LASSO$lambda.min #Initial candidate
    lambdamax1<-try(optim.lam(X,Y,beta.init, weight.mode,LR.loss,lambdamax0, alpha)) #Improvement
...
optim.lam<-function(X,Y,beta.init, weight.mode, LR.loss,lambdamax, alpha){
